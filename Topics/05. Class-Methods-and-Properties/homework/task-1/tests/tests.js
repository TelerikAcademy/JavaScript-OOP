...
describe('Linked list: ', () => {
    ...
    it('should insert correctly', () => {
        ...
        expect(list.last).to.equal(2); // changed
        expect(list.length).to.equal(5);
        expect(list.toString()).to.equal([3, 4, 1, 'kremikovci', 2].join(' -> ')); // changed
    });
    ...
    it('should have correct removeAt', () => {
        ...
        const values = ['test', true, null, 1, 2, 'testtest', theObj, 'gg'], //changed
            list = new LinkedList().append(...values),
        ...
        expect(list.first).to.equal(1);
        expect(list.last).to.equal(theObj); //changed

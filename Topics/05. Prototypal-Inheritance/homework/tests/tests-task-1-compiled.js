...
describe('Tests for "Task 1"', function () {
	...
	describe('Sanity checks', function () {
		...
		it('expect adding attribute with empty name to throw', function () {
			function test() {
				var root = Object.create(domElement).init('whatever').addAttribute('', 'spam');
			}
			expect(test).to['throw']();
		});
		it('expect adding attribute with name containing bad characters to throw', function () {
			function test() {
				var root = Object.create(domElement).init('whatever').addAttribute('Hello 007', 'spam');
			}
			expect(test).to['throw']();
		});
		it('expect adding attribute with valid name not to throw', function () {
			function test() {
				var root = Object.create(domElement).init('whatever').addAttribute('data-spam', 'mnogo spam');
			}
			expect(test).to.not['throw']();
		});
		...
		it('expect removing non-existent attribute to throw', function () {
			function test() {
				var root = Object.create(domElement).init('td').removeAttribute('colspan');
			}
			expect(test).to['throw']();
		});
		it('expect removing non-existent attribute to throw', function () {
			function test() {
				var root = Object.create(domElement).init('td').addAttribute('rowsomething', 'has a value').removeAttribute('colspan');
			}
			expect(test).to['throw']();
		});
		it('expect removing existent attribute not to throw', function () {
			function test() {
				var root = Object.create(domElement).init('td').addAttribute('rowsomething', 'has a value').removeAttribute('rowsomething');
			}
	...
	describe('Adding attributes', function () {
		...
		it('expect attribute value to be in double quotes when it is a number', function () {
			var root = Object.create(domElement).init('whateverTag').addAttribute('something', 42);
		...
		it('expect domElement with attributes to generate HTML with the attributes in correct order', function () {
			var root = Object.create(domElement).init('NqmaTakufTag').addAttribute('xyz', 'some value').addAttribute('zzz', '').addAttribute('abc', 'other value');
		...
		it('expect repeating attribute name to hold only the last value', function () {
			var root = Object.create(domElement).init('theGuiltyTag').addAttribute('adata', 'do not').addAttribute('bdata', 'see').addAttribute('adata', 'me');
	...
	describe('Parent control', function () {
		it('expect children to know their parents (one parent, one child)', function () {
			var child = Object.create(domElement).init('child'),
			    parent = Object.create(domElement).init('parent').appendChild(child);
		...
		it('expect children to know their parents (one parent, two children)', function () {
			var child1 = Object.create(domElement).init('child'),
			    ...
			    parent = Object.create(domElement).init('parent').appendChild(child1).appendChild(child2);
		...
		it('expect children to know their parents (grandparent, parent and child)', function () {
			var child = Object.create(domElement).init('child'),
			    middle = Object.create(domElement).init('middleblq').appendChild(child),
			    parent = Object.create(domElement).init('parent').appendChild(middle);
	...
	describe('Adding children', function () {
		it('expect correct HTML when child is a string', function () {
			var text = 'Some text here, doesn\'t really matter that much what it is.',
			    root = Object.create(domElement).init('p').appendChild(text);
		...
		it('expect string children to override parent\'s content', function () {
			var text = 'see me',
			    parent = Object.create(domElement).init('parent').appendChild(text);
		...
		it('expect correct HTML, when there are attributes with same names, but in different domElements', function () {
			var child = Object.create(domElement).init('child').addAttribute('id', 'cid'),
			    parent = Object.create(domElement).init('parent').addAttribute('id', 'pid').appendChild(child);
	...
	describe('Removing attributes', function () {
		it('expect removing an attribute to work (removing the only attribute)', function () {
			var root = Object.create(domElement).init('table').addAttribute('style', 'something: beautiful').removeAttribute('style');

			expect(root.innerHTML).to.eql('<table></table>');
		});
		it('expect removing an attribute to work (removing one of the attributes)', function () {
			var root = Object.create(domElement).init('table').addAttribute('style', 'something: beautiful').addAttribute('id', 'the_id').removeAttribute('style').addAttribute('class', 'the_class');
		...
		it('expect removing an attribute to work (removing attribute that was added twice)', function () {
			var root = Object.create(domElement).init('table').addAttribute('class', 'word').addAttribute('class', 'two words').addAttribute('style', 'something: beautiful').removeAttribute('class');
		...
		it('expect removing an attribute to work (removing an attribute and then adding it again)', function () {
			var root = Object.create(domElement).init('table').addAttribute('class', 'word').removeAttribute('class').addAttribute('class', 'word');

			expect(root.innerHTML).to.eql('<table class="word"></table>');
		});

		it('expect removing an attribute to work (nested domElements)', function () {
			var parent = Object.create(domElement).init('html').addAttribute('id', 'myindex.html'),
			    child = Object.create(domElement).init('head').addAttribute('id', 'myhead');
	...
	describe('Mixing it all together', function () {
		it('expect the example test given in the description to work', function () {
			var meta = Object.create(domElement).init('meta').addAttribute('charset', 'utf-8'),
			    head = Object.create(domElement).init('head').appendChild(meta),
			    div = Object.create(domElement).init('div').addAttribute('style', 'font-size: 42px');

			div.content = 'Hello, world!';

			var body = Object.create(domElement).init('body').appendChild(div).addAttribute('id', 'myid').addAttribute('bgcolor', '#012345'),
			    root = Object.create(domElement).init('html').appendChild(head).appendChild(body);
		...
		it('expect this big test to work (no removeAttribute)', function () {
			var style = Object.create(domElement).init('style').appendChild('#big {\nfont-size: 144pt;\n}'),
			    link = Object.create(domElement).init('link').addAttribute('src', 'css/fancy.css'),
			    meta = Object.create(domElement).init('meta').addAttribute('charset', 'utf-8'),
			    title = Object.create(domElement).init('title').appendChild('Super-Mega awesome S173'),
			    script = Object.create(domElement).init('script').addAttribute('lang', 'javascript').appendChild('function init(){}'),
			    head = Object.create(domElement).init('head').appendChild(meta).appendChild(title).appendChild(link).appendChild(style).appendChild(script),
			    heading = Object.create(domElement).init('h1'),
			    luser = Object.create(domElement).init('label').addAttribute('for', 'username').addAttribute('class', 'big'),
			    lpass = Object.create(domElement).init('label').addAttribute('for', 'password'),
			    user = Object.create(domElement).init('input').addAttribute('name', 'username').addAttribute('id', 'username').addAttribute('type', 'input').addAttribute('tab-index', 1),
			    pass = Object.create(domElement).init('input').addAttribute('name', 'password').addAttribute('id', 'password').addAttribute('type', 'password').addAttribute('tab-index', 2),
			    submit = Object.create(domElement).init('input').addAttribute('type', 'submit').addAttribute('value', 'natis'),
			    form = Object.create(domElement).init('form').appendChild(luser).appendChild(user).addAttribute('action', 'vlez/mi/u/profila').appendChild(lpass).addAttribute('method', 'post').appendChild(pass).appendChild(submit),
			    footer = Object.create(domElement).init('footer'),
			    body = Object.create(domElement).init('body').appendChild(heading).appendChild(form).appendChild('reklamata').appendChild(footer),
			    html = Object.create(domElement).init('html').appendChild(head).appendChild(body);
